...
public struct NetworkError: Equatable {
    public enum ErrorStatus: Int, Error, Equatable {
        case unknown                                      = -1
    ...
    init(error: Error) {
        ...
        if let status = error as? ErrorStatus {
            self.errorStatus = status
            return
        }
        
        if let urlError = error as? URLError {
            self.errorStatus = ErrorStatus(rawValue: urlError.errorCode) ?? .unknown
            return
        }
        
        self.errorStatus = .unknown
    }
    
    init(errorCode: Int) {
        self.errorStatus = ErrorStatus(rawValue: errorCode) ?? .unknown
        errorMessage = errorStatus.localizedDescription
    }
}
